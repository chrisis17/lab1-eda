...
class Boid:
    ...
    # Separation force
    # Si la distancia entre el boid y el vecino es menor que la distancia de separación, entonces se añade la fuerza de separación
    # La fuerza de separación es la dirección desde el vecino al boid
    # La fuerza de separación se limita a la magnitud de la fuerza máxima
    # Esto evita que los boids se amontonen
    #   - neighbors:     (list) lista de vecinos
    #   - return   : (np.array) fuerza de separación
    ...
    # Alignment force
    # Si la distancia entre el boid y el vecino es menor que la distancia de alineación, entonces se añade la fuerza de alineación
    # La fuerza de alineación es la dirección de la velocidad del vecino
    # La fuerza de alineación se limita a la magnitud de la fuerza máxima
    # Esto evita que los boids se muevan en direcciones opuestas
    #  - neighbors:     (list) lista de vecinos
    #  - return   : (np.array) fuerza de alineación
    def alignment(self, neighbors):
        ...
        if count > 0:
            steering /= count
            steering = self._limit_force(steering - self.velocity)
        return steering

    # Cohesion force
    # Si la distancia entre el boid y el vecino es menor que la distancia de cohesión, entonces se añade la fuerza de cohesión
    # La fuerza de cohesión es la dirección desde el boid al vecino
    # La fuerza de cohesión se limita a la magnitud de la fuerza máxima
    # Esto evita que los boids se muevan en direcciones opuestas
    #   - neighbors:     (list) lista de vecinos
    #   - return   : (np.array) fuerza de cohesión
    def cohesion(self, neighbors):
        ...
        if count > 0:
            steering /= count
            steering = self._limit_force(steering - self.position)
        return steering
